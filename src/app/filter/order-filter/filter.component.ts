...
export class FilterComponent implements OnInit {
  ...
  selectedDataToFilter: any = { district: [],
    place: [], agency_name: [],
  ...
  indentList() {
    ...
    let httpOptions = {
     ...
     this.http
        .post('http://134.209.147.129:3001/list_order', request, httpOptions)
        .toPromise()
        .then((res: any) => {
            console.log('result order', res.message);
            /// return res;
           this.orderService._order_list$.next(res.message);
        })
  ...
  setAgencyFilter() {
   ...
   submitFilter() {
    ...
    console.log('selectedDataToFilter', this.selectedDataToFilter);
     this.orderService.filterOrder(this.selectedDataToFilter);
